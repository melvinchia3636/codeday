...
export class WaterLogService extends BaseService<WaterLog> {
   ...
   */
  async shouldDrinkWater(userId: string): Promise<{ shouldDrink: boolean; lastDrink: Date | null; minutesSinceLastDrink: number }> {
    const todayLogs = await this.getTodayLogs(userId);
    ...
    const now = new Date();
    const minutesSinceLastDrink = Math.floor((now.getTime() - lastDrinkTime.getTime()) / (1000 * 60));
   ...
   */
  async getTodaySummary(userId: string, targetMl: number = 2000): Promise<{
    totalMl: number;
    targetMl: number;
    percentage: number;
    logsCount: number;
    ...
    return {
      ...
      minutesSinceLastDrink: drinkStatus.minutesSinceLastDrink === Infinity ? 0 : drinkStatus.minutesSinceLastDrink
    };
  }
