...
export class WaterLogController extends BaseController<WaterLog> {
  ...
  async getTodayLogs(
    ...
    try {
      const userId = req.userId!;
      const logs = await this.waterLogService.getTodayLogs(userId);
  ...
  async getSummary(
    ...
    try {
      const userId = req.userId!;
      const targetMl = parseInt(req.query.targetMl as string) || 2000;
      const summary = await this.waterLogService.getTodaySummary(userId, targetMl);
