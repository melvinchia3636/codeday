...
interface UseSignupAnimationsProps {
  ...
  useEffect(() => {
    ...
    if (glowBorderRef.current) {
      animate(glowBorderRef.current, {
        background: [
          "linear-gradient(0deg, rgba(34,211,238,0.3), rgba(168,85,247,0.3), rgba(236,72,153,0.3))",
          "linear-gradient(90deg, rgba(34,211,238,0.3), rgba(168,85,247,0.3), rgba(236,72,153,0.3))",
          "linear-gradient(180deg, rgba(34,211,238,0.3), rgba(168,85,247,0.3), rgba(236,72,153,0.3))",
          "linear-gradient(270deg, rgba(34,211,238,0.3), rgba(168,85,247,0.3), rgba(236,72,153,0.3))",
          "linear-gradient(360deg, rgba(34,211,238,0.3), rgba(168,85,247,0.3), rgba(236,72,153,0.3))",
        ],
        duration: 4000,
        ease: "linear",
        loop: true,
    ...
    if (hexGridRef.current) {
      for (let i = 0; i < 20; i++) {
        ...
        hex.className = "absolute border border-cyan-500/10 rotate-45";
        hex.style.width = `${random(15, 50)}px`;
        hex.style.height = `${random(15, 50)}px`;
    ...
    if (dataStreamsRef.current) {
      for (let i = 0; i < 10; i++) {
        ...
        stream.className =
          "absolute h-px bg-gradient-to-r from-transparent via-cyan-500/40 to-transparent";
        stream.style.width = `${random(50, 150)}px`;
    ...
    const glitchInterval = setInterval(() => {
      if (logoRef.current) {
        ...
        animate(logoRef.current, {
          textShadow: [
            "0 0 0 transparent",
            "-2px 0 0 rgba(34,211,238,0.5), 2px 0 0 rgba(236,72,153,0.5)",
            "0 0 0 transparent",
          ],
          duration: 200,
          ease: "inOutQuad",
...
export function useInputAnimations() {
  const handleInputFocus = (e: React.FocusEvent<HTMLInputElement>) => {
    animate(e.target, {
      ...
      borderColor: ["rgba(34, 211, 238, 0.3)", "rgba(34, 211, 238, 1)"],
      duration: 300,
      ease: "outQuad",
    });
